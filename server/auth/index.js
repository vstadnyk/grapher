...
const { tokens, headerKey, deadTokenHeaderStatus } = jwtConfig
...
export default class Auth {
	...
	/* 
		NOTICE! NON SECURE!
			refresh.secret never change, it take from config file
		
		SOLUTION
			kip refresh.secret in ctx and change with this method
	*/
	async refreshToken(refreshToken) {
		...
		if (!_user || !_user.mail)
			throw new Error(
				`Auth error: Unable parse refreshToken data "${JSON.stringify(
					_user
				)}"`
			)
		...
		if (!user)
			throw new Error(
				`Auth error: User with mail "${_user.mail}" not found`
			)

		if (_user.mail && _user.mail !== user.mail)
			throw new Error(
				`Auth error: Not equal mail "${_user.mail}" !== "${user.mail}"`
			)
	...
	afterPassport() {
		return async (ctx, next) => {
			...
			if (this.ctx.get(headerKey) && !(await this.validateToken()))
