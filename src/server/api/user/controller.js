...
export default class extends Controller {
	...
	async changePass(data) {
		...
		if (!data.rid)
			throw new Error(this.locale.get(`${this.table}_error_change_pass`))
	...
	async user(where = false, options, filter = row => row) {
		if (!where)
			throw new Error(this.locale.get(`${this.table}_error_arguments_required`))
	...
	async removeUsers(where, errors = true) {
		...
		if (!list) throw new Error(this.locale.get(`${this.table}_error_not_found`))
